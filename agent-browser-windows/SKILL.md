...
| # | 规则 | 验证 |
|---|------|------|
| 1 | **防卡住超时 wrapper（最高优先级）**：每个 agent-browser 命令必须包在可靠的超时里执行。`open`/`wait` 连不上目标或等页面加载时，agent-browser 自身 timeout 不生效，实测可卡 138s+，导致 subagent 执行链阻塞无法自救。必须用 PowerShell `Start-Process` + `WaitForExit(timeout)` + 超时 `taskkill /T /F` 包裹（**不可用** Git Bash `timeout` 命令——它发 SIGTERM 杀不掉 Windows 原生进程）。见 `Invoke-AB` | 命令经 `Invoke-AB`（ab-primitives.ps1）执行；wrapper 含 `WaitForExit` + 超时 kill |
| 2 | **全局串行化锁**：任何 agent-browser 命令前必须先获取全局锁，确保同一时间只有一个 session 在工作。多个 subagent/命令必须排队等待。这是根治并发的关键（官方 #326/#1378/#885 确认并发是冲突主因） | 工作流以 `Acquire-Lock` 开始、`Release-Lock` 结束；见 ab-primitives.ps1 |
...
| 5 | **close 后必须验证零残留**：独立调用检查该 session 的 `.pid` sidecar 已消失、对应 daemon PID 已死 | 验证输出 `closed cleanly`，或列出残留 |
| 6 | **等待优先具体条件**：优先 `wait @ref` / `wait --text "..."` / `wait --url "**/path"`；`networkidle` 仅作兜底，不设为默认；禁止裸 `wait <ms>` | wait 命令非裸 `wait <ms>` |
| 7 | **孤儿清理必须双重校验**：清理 Chrome 必须同时校验可执行路径标记 **和** `agent-browser-chrome-` user-data-dir；绝不按进程名单杀；daemon 用 `.pid` + 进程名双校验 | cleanup-orphans.ps1 含两处路径校验 |
| 8 | **清理前后必须计数对比**：清理脚本输出清理前后的 daemon/chrome 进程数，证明有效 | cleanup-orphans.ps1 输出 before/after 计数 |
| 9 | **场景脚本的使用时机**：cleanup-orphans/cleanup-deep/diagnose 是**事前预防/事后清理**工具，不是卡住自救工具（卡住时执行链阻塞，无法调用它们）。在工作流开始前（预防性清理残留）或超时/崩溃恢复后（事后清理）使用 | 非在卡住的 bash 调用内使用 |
...
### 标准工作流（锁 → open → 业务步骤 → close → 验证 → 释放锁）

中间业务步骤（snapshot/click/wait）交互式，逐条用 `Invoke-AB` 执行。`try/finally` 保证锁一定释放：

```powershell
. <skill-dir>/scripts/ab-primitives.ps1

$SESSION = "wallet-qa"                              # ← 任务相关且唯一的 session 名
$URL     = "http://127.0.0.1:3000/wallet"           # ← 目标 URL
$env:AGENT_BROWSER_IDLE_TIMEOUT_MS = "60000"        # idle timeout 安全网（规则 10）

if (-not (Acquire-Lock -Session $SESSION -MaxWaitSec 120)) { Write-Output "ERROR: lock"; exit 1 }
try {
    $r = Invoke-AB -Session $SESSION -ABArgs @('open', $URL) -TimeoutSec 30
    if ($r.TimedOut) { throw "open timed out" }
    # 业务步骤（按需）— 全部经 Invoke-AB
    Invoke-AB -Session $SESSION -ABArgs @('snapshot', '-i', '-d', '4') -TimeoutSec 15
    # Invoke-AB -Session $SESSION -ABArgs @('click', '@e3') -TimeoutSec 10
    # Invoke-AB -Session $SESSION -ABArgs @('wait', '--url', '**/success') -TimeoutSec 20
    $r = Invoke-AB -Session $SESSION -ABArgs @('close') -TimeoutSec 10
} finally {
    Start-Sleep -Milliseconds 500
    $pidFile = "$env:USERPROFILE\.agent-browser\${SESSION}.pid"
    if (Test-Path $pidFile) {                                    # 验证零残留（规则 5）
...
# ✅ 必须经 Invoke-AB wrapper（dot-source ab-primitives.ps1）

# ❌ 并发调用无锁 —— session 劫持(#326)、Chrome 静默崩溃(#1378)、孤儿堆积
# ✅ 工作流首尾 Acquire-Lock / Release-Lock

# ❌ 裸 open 无 session —— 用了 default session，状态串台
# ✅ --session <task-name>

# ❌ 忘记 close 或只靠 IDLE_TIMEOUT —— 锁被占满计时（如 60s），排队者全阻塞
# ✅ 工作完成立即 close 释放锁

# ❌ 按进程名杀 chrome —— 会杀掉用户真实 Chrome
taskkill /im chrome.exe /F
# ❌ taskkill 不带 /T —— Chrome 子进程（renderer/gpu）全变孤儿
# ❌ PowerShell 正则用 [\\/] 匹配反斜杠 —— 实测对 \ 失败，校验静默失效
# ✅ 用 cleanup-orphans.ps1（agent-browser.+browsers 通配分隔符 + 双重校验）

# ❌ 浏览器卡住时杀掉目标端口上的应用进程 —— 杀错对象
# ✅ 先 diagnose.ps1 区分浏览器 daemon PID 和被访问应用 PID
```
