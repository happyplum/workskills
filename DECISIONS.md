...
本文件记录持续影响本仓 skill 行为的用户决策与行为裁决，是本仓唯一的决策入口：修改任何 skill 行为前先核对其中的 `active` / `superseded` 状态，避免回退到已废弃方向。纯维护规范（写作标准、提交流程、结构约定）记录在 README，不记录于此。
本文件 2026-08-23 从两仓共用决策体制拆分建立（此前 skill 侧决策记录在 prompts 仓的 DECISIONS.md），编号独立、条目自包含；两仓各自演进，互不依赖对方文件。
...
- 状态：`active`
- 决策：删除逐任务验收仪式——预算内持续 fan-out 补位，不因等待验收阻塞无依赖 ready task；验收集中在 wave 末、检查点、依赖解锁前、终态排水四类节点；预算口径（运行中写入 worker + 未验收产物，默认 3、隔离充分至 4、计划 `concurrency_budget` 唯一覆盖）为强制背压；公共接口、持久化、安全、并发、迁移、不可逆等高风险边界完成即验收。参照 Codex/Claude 编排模式（并行 fan-out + 统一收集）与「orchestrator 频繁打断子代理」的反面教训。
