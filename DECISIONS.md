...
- 修订（2026-08-24，SK-007）：预算体制限定为计划路径；Sisyphus 日常路径的并发节奏由 overlay 单批蜂群条款承接（SK-007），强制背压思想不变——单批在飞上限即该路径的排水门。
...
- 状态：`active`
- 决策：`omo-adaptive-execution` 的波次制与并发预算（默认 3/4，`concurrency_budget` 唯一覆盖）为**计划路径**（Atlas/矩阵）节奏；例外条款**仅限 Sisyphus overlay（日常任务路径）**显式声明的单批蜂群模式可覆盖节奏与数值上限（单批在飞 6，批末统一验收后清零）。不豁免项：写域互斥、命名依赖串行（后继读取前驱产物、同文件写入）、验收门（批末统一验收 + 高风险边界完成即验）、Category 路由与升档协议、发现委托、质量门。SK-003 的最小化拆解与「共享推理不拆」边界不变；SK-004 的预算体制在 Sisyphus 路径由 overlay 蜂群条款承接。
