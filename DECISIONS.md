...
### SK-020 完成审查 metis 委托证据基准与目的核对入口

- 状态：`active`
- 决策（2026-09-04，prompts 仓 D-048 同裁决）：`omo-atlas-execution-constraints`「完成审查」节——①先由 `metis` 检查遗漏、隐藏问题和范围偏差的委托必须附对照基准（用户需求原话或计划「需求与目标」节摘录、产物摘要），无基准时不做范围偏差核对（范围偏差核对自此有对照依据，不凭印象）；②用户显式要求目的核对时委托前台 `metis` 并附同样证据，输入要求以 `metis.md` 输入契约为准（目的差异分析判据与输入契约单一来源在 prompts 仓 `metis.md`，本仓不复制）；用户明确拒绝即跳过。
- 验收：完成审查节含证据基准、无基准降级句与显式触发入口；判据与输入契约不在本仓重复维护。
- 修订（2026-09-05，SK-021）：完成审查的差异分析委托传参规范由 SK-021 承载于 `omo-adaptive-execution`「委托契约」节；三判据与「输入契约」（响应方验收标准）仍在 prompts 仓 `metis.md`。

### SK-021 Metis 目的差异分析传参契约（skill 承载）

- 状态：`active`
- 决策（2026-09-05，用户裁决「metis 要保留输入契约，其他智能体加载的应该是传参契约，需要把合理的资料提供给 metis」，prompts 仓 D-048 修订同裁决）：`omo-adaptive-execution`「委托契约」节新增「Metis 目的差异分析传参契约」段——派发方组装目的差异分析委托参数的规范（用户原话按轮次引号摘录、转述标注不混排；被分析方文本原文摘录，不凭标题或复述替代；最低资料量：目的着重点≥两轮原话、描述清晰度=原话＋解读文本、符合度=陈述条目＋被分析方文本）与退回重发语义（参数不满足输入契约的委托被 metis 退回所需证据清单、补齐后重发）；仅约束目的差异分析类委托，执行图生成、完成审查查遗漏等既有 metis 委托不适用。`metis.md` 保留「输入契约」节（响应方验收标准）——**传参契约与输入契约是同一委托接口的两端，各自自包含、内容对应，不视为重复维护**。
- 验收：`omo-adaptive-execution` 委托契约节含该段；三调用点引用已改指传参契约；`metis.md` 含「输入契约」节；两端条款语义一致（最低资料量＝最低输入）。
