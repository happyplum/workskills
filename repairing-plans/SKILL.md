...
**规范化入口：** 对 reviewed / imported plan 先分类为 `direct-execute`、`normalize-before-execute` 或 `repair-before-execute`。若只是缺本地治理约束、仍可确定恢复，则先规范化；若存在硬关卡失败，再进入修复。
...
7. **平台到本地的收敛**：上游平台 runtime 术语（如 `sisyphus-junior`）可以作为 imported plan 的事实输入，但若超出本地 authoring subset，必须先被规范化成本地可执行的路由表达，不能直接越过本地 schema 进入执行。
