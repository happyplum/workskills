...
5. **治理写入并发**：写入 worker 默认并发预算为 3；当工作区、写入资源、共享接口和验收 owner 均隔离充分时，不设固定数量上限。默认值是保守门槛，不是配额或硬上限；有几个相互独立的 ready 流就派几个，无法证明隔离的 ready 保持 pending。只读发现不计入写入并发。WIP 仍统计运行中 worker 与已完成未验收产物，用于暴露验收积压而非形成数量上限；同一 ready cohort 在隔离充分时同波派发，否则按可验证隔离边界分批，cohort 归属不受分批影响。独立 ready 写入任务默认后台并发；只有立即后继依赖其结论时才同步等待。
