...
description: 当当前代理承担 OMO 协调者角色（Sisyphus 或 Atlas），且用户请求修改文件或配置、实现、修复、重构、迁移，或需要拆分、委托、并行、派发 task 时必须加载；纯解释、问答、审查及不协调子代理的只读诊断不加载。
---
...
5. **限制写入并发**：写入 worker **上限**默认 3、隔离充分时最多 4——是 **cap 不是配额**；有几个独立 ready 流就派几个。额外 ready 保持 pending。只读发现不计入写入上限。
...
4. **DISPATCH**：按关键路径派发写入 task（上限 3/4，不是配额），其余 pending。
