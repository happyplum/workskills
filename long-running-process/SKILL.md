...
| 3 | **就绪检查必须独立、有界、失败 `exit 1`**：用独立 bash 调用检查端口/health endpoint，基于框架预算设超时 | `start-background` 与 `wait-ready` 分两个独立 bash call；`wait-ready.ps1` 超时 `exit 1` |
...
> **start-background 与 wait-ready 必须分两个独立 bash 调用**（规则 3）。`cleanup-port` 的 `-RequireMatch` 必须是绝对项目路径或唯一标记——不可用 `node`/`pnpm` 等裸可执行名。
