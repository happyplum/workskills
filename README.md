...
| [omo-plan-structure](omo-plan-structure/) | OMO 计划结构单一标准：五区块 schema、矩阵结构约束、任务原子性契约、并行准入标准、Task 契约字段与计划/账本分离；Prometheus 编写、Momus 审查与 Atlas 执行前必须加载 |
...
omo-plan-structure ──→ 计划结构单一标准（Prometheus 编写 / Momus 审查 / Atlas 执行前加载，prompt 不复制结构定义）
