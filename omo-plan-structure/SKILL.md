...
description: 当当前代理承担 Prometheus（编写或修订计划）、Momus（审查计划版本）或 Atlas（读取与执行计划）角色，即将开始规划工作、审查任何计划版本或执行协调前必须加载；只解释规则、问答，不产出、核对或执行具体计划时不加载。
---
...
本 skill 是 OMO 计划文件结构的**单一标准**：计划正文的区块构成、各区块必需内容、结构约束、任务原子性契约与计划/账本分离体制均以本文件为准。编写方（Prometheus）、审查方（Momus）与执行协调方（Atlas）共用本标准，任何一方不得另行复制或改写结构定义；结构类不一致以本文件裁决。生成方法、审查裁决规则与执行协调职责由各自 prompt 承载，不在此复制。
